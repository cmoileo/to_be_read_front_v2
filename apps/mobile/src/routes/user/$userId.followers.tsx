...
interface FollowersSearchParams {
  userName?: string;
}

export const Route = createFileRoute("/user/$userId/followers")({
  beforeLoad: async () => {
  ...
  validateSearch: (search: Record<string, unknown>): FollowersSearchParams => {
    return {
      ...
      <PageTransition className="flex-1 pb-20">
        <div className={`p-4 border-b flex items-center gap-3 ${isMobile ? 'pt-[calc(env(safe-area-inset-top)+1rem)]' : ''}`}>
          ...
          <h1 className="text-lg font-semibold">
            {t("followList.followersTitle", { userName: userName || "" })}
          </h1>
        </div>
