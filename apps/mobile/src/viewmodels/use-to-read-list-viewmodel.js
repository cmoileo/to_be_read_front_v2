...
import { queryKeys, addBookToListCache, removeBookFromListCache, rollbackAddBook, rollbackRemoveBook, initializeBookIdsFromList, isBookInListCache, } from "@repo/stores";
export const useToReadListViewModel = () => {
    ...
    const addMutation = useMutation({
        mutationFn: (googleBookId) => MobileToReadListService.addToReadList(googleBookId),
        onMutate: async (googleBookId) => {
    ...
    const removeMutation = useMutation({
        mutationFn: (googleBookId) => MobileToReadListService.removeFromReadList(googleBookId),
        onMutate: async (googleBookId) => {
    ...
    const handleAddToList = useCallback((googleBookId) => {
        return addMutation.mutateAsync(googleBookId);
    }, [addMutation]);
    const handleRemoveFromList = useCallback((googleBookId) => {
        return removeMutation.mutateAsync(googleBookId);
    }, [removeMutation]);
    const isInList = useCallback((googleBookId) => {
