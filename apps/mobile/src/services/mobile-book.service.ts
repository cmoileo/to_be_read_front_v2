...
export class MobileBookService {
  static async getBook(googleBookId: string): Promise<GoogleBook> {
    return HttpInterceptor.get<GoogleBook>(`/book/${googleBookId}`);
  }

  static async getBookReviews(googleBookId: string, page: number = 1): Promise<BookReviewsPaginatedResponse> {
    const response = await HttpInterceptor.get<{
      data: ApiBookReview[];
      meta: { total: number; perPage: number; currentPage: number; lastPage: number };
    }>(`/book/${googleBookId}/reviews/${page}`);
